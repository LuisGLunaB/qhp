...
class SQLBasicTableManager{
  ...
  private function isFieldsMaskOn(){
    return !is_null($this->fieldsMask);
  }
  public static function maskArray(array $maskArray, array $validArray){
    $maskedArray = [];
    foreach($maskArray as $mask){
      if( in_array($mask,$validArray) ){
        $maskedArray[] = $mask;
      }
    }
    return $maskedArray;
  ...
  private function isTableFieldsNULL(){
    return is_null($this->TableFields);
  }
  private function isTablesNamesNULL(){
